...
int main()
{
	...
	for(i=0; i<1000000*R; i++)
	{
		...
		if(b)
		{
			...
			if(cabs(p) <= e_out)
			{	
				g = Gamma(repol, impol); //Computes gamma (p, z), where z is the candidate to the computed solution by Newton iteration.    		
				for(j=0; j<=t; j++)
				{	
					if(gam[j] > g )
					{
					...
					if(cabs(z-sol[j]) < C/max) //It verifies if the computed solution is very close to another one already computed. If the difference is less than this bound, then it is the same solution.
					{	
						b = 0;
						break;
					}
